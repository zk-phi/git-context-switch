...
(defun -mv/cp-internal (from to &optional copy)
  (let* ((from (split-string from ":"))
         ...
         (from-prefix (context-prefix (and (not (context-active-p from-context)) from-context)))
         (from-ref (concat from-prefix "heads/" from-branch))
         (to (split-string to ":"))
         (to-context (pop to))
         (to-branch (or (pop to) from-branch))
         (to-ref (concat (context-prefix to-context) "heads/" to-branch)))
    (unless (context-exists-p from-context)
      (error "Context %s does not exist." from-context))
    (unless (context-exists-p to-context)
      (error "Context %s does not exist." to-context))
    (let ((current-head (head (shell-command-to-string-noerror "git symbolic-ref HEAD"))))
